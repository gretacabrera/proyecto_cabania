    ...
    <div class="card border-0 shadow-sm">
        ...
        <div class="card-header text-dark py-3 mb-0">
            <div class="row align-items-center">
                <div class="col">
                    <h4 class="mb-0">Gestión de Estados de Consumo</h4>
                </div>
                <div class="col-auto">
                    <a href="<?= url('/estadosconsumo/create') ?>" class="btn btn-primary btn-sm">
        ...
        <div class="card-body pb-0">
            <form method="GET" action="<?= url('/estadosconsumo') ?>" class="mb-3">
                <div class="row g-2 align-items-end">
                    ...
                    <div class="col-auto">
                        ...
                        <input type="text" name="estadoconsumo_descripcion" class="form-control form-control-sm" 
                               placeholder="Buscar..." value="<?= htmlspecialchars($_GET['estadoconsumo_descripcion'] ?? '') ?>" style="width: 200px;">
                    </div>
                    <div class="col-auto ms-auto">
                        <label class="form-label small mb-1">Estado</label>
                        <select name="estadoconsumo_estado" class="form-select form-select-sm" style="width: 120px;">
                            <option value="">Todos</option>
                            <option value="1" <?= ($_GET['estadoconsumo_estado'] ?? '') == '1' ? 'selected' : '' ?>>Activo</option>
                            <option value="0" <?= ($_GET['estadoconsumo_estado'] ?? '') == '0' ? 'selected' : '' ?>>Inactivo</option>
                        </select>
                    </div>
                    <div class="col-auto">
                        <div class="btn-group">
                            ...
                            <a href="<?= url('/estadosconsumo') ?>" class="btn btn-info btn-sm" title="Limpiar filtros">
        ...
        <div class="card-body p-0">
            <?php if (empty($estados)): ?>
                <div class="empty-state py-5 text-center">
                    ...
                    <h6 class="text-muted">No se encontraron estados de consumo</h6>
                    <p class="text-muted small mb-3">Intenta modificar los filtros o crea un nuevo estado.</p>
                    <a href="<?= url('/estadosconsumo/create') ?>" class="btn btn-outline-dark btn-sm">
                            ...
                            <?php foreach ($estados as $estado): ?>
                                <tr>
                                    <td class="border-0 py-3">
                                        <div class="fw-medium text-dark"><?= htmlspecialchars($estado['estadoconsumo_descripcion']) ?></div>
                                    </td>
                                    <td class="border-0 py-3">
                                    ...
                                    <td class="border-0 py-3 text-center">
                                        <div class="btn-group btn-group-sm" role="group">
                                            <a href="<?= url('/estadosconsumo/' . $estado['id_estadoconsumo']) ?>"
...
function exportarEstados(event) {
    ...
    const baseExportUrl = '<?= url('/estadosconsumo/exportar') ?>';
...
function exportarEstadosPDF(event) {
    ...
    const urlParams = new URLSearchParams(window.location.search);
    const basePdfUrl = '<?= url('/estadosconsumo/exportar-pdf') ?>';
