    ...
    <div class="card border-0 shadow-sm">
        ...
        <div class="card-header text-dark py-3 mb-0">
            <div class="row align-items-center">
                <div class="col">
                    <h4 class="mb-0">Gestión de Ubicaciones</h4>
                </div>
                <div class="col-auto">
                    <a href="<?= url('/ubicaciones/create') ?>" class="btn btn-primary btn-sm">
        ...
        <div class="card-body pb-0">
            <form method="GET" action="<?= url('/ubicaciones') ?>" class="mb-3">
                <div class="row g-2 align-items-end">
                    ...
                    <div class="col-auto">
                        ...
                        <input type="text" name="ubicacion_descripcion" class="form-control form-control-sm" 
                               placeholder="" value="<?= htmlspecialchars($_GET['ubicacion_descripcion'] ?? '') ?>" style="width: 250px;">
                    </div>
                    <div class="col-auto ms-auto">
                        <label class="form-label small mb-1">Estado</label>
                        <select name="ubicacion_estado" class="form-select form-select-sm" style="width: 120px;">
                            <option value="">Todos</option>
                            <option value="1" <?= ($_GET['ubicacion_estado'] ?? '') == '1' ? 'selected' : '' ?>>Activo</option>
                            <option value="0" <?= ($_GET['ubicacion_estado'] ?? '') == '0' ? 'selected' : '' ?>>Inactivo</option>
                        </select>
                    </div>
                    <div class="col-auto">
                        <div class="btn-group">
                            ...
                            <a href="<?= url('/ubicaciones') ?>" class="btn btn-info btn-sm" title="Limpiar filtros">
                ...
                <div class="row mt-4">
                    ...
                    <div class="col-auto">
                        <div class="btn-group" role="group">
                            <button type="button" onclick="exportarUbicaciones(event)" class="btn btn-success btn-sm" title="Exportar a Excel">
                                <i class="fas fa-file-excel me-1"></i> Excel
                            </button>
                            <button type="button" onclick="exportarUbicacionesPDF(event)" class="btn btn-danger btn-sm" title="Exportar a PDF">
        ...
        <div class="card-body p-0">
            <?php if (empty($ubicaciones)): ?>
                <div class="empty-state py-5 text-center">
                    ...
                    <h6 class="text-muted">No se encontraron ubicaciones</h6>
                    <p class="text-muted small mb-3">Intenta modificar los filtros o crea una nueva ubicación.</p>
                    <a href="<?= url('/ubicaciones/create') ?>" class="btn btn-outline-dark btn-sm">
                        <i class="fas fa-plus fa-sm"></i> Crear ubicación
                    </a>
                </div>
            <?php else: ?>
                ...
                <div class="table-responsive">
                    <table id="tablaUbicaciones" class="table table-hover mb-0">
                                ...
                                <tr>
                                    ...
                                    <td class="border-0 py-3 text-center">
                                        <div class="btn-group btn-group-sm" role="group">
                                            <a href="<?= url('/ubicaciones/' . $ubicacion['id_ubicacion']) ?>"
                                            ...
                                            <?php if ($ubicacion['ubicacion_estado'] == 1): ?>
                                                <button class="btn btn-outline-danger btn-sm"
                                                        onclick="cambiarEstadoUbicacion(<?= $ubicacion['id_ubicacion'] ?>, 0, '<?= addslashes($ubicacion['ubicacion_descripcion']) ?>')"
                                                        title="Desactivar">
                                                    <i class="fas fa-ban"></i>
                                                </button>
                                            <?php else: ?>
                                                <button class="btn btn-outline-success btn-sm"
                                                        onclick="cambiarEstadoUbicacion(<?= $ubicacion['id_ubicacion'] ?>, 1, '<?= addslashes($ubicacion['ubicacion_descripcion']) ?>')"
                                                        title="Activar">
...
<script>
function cambiarEstadoUbicacion(id, nuevoEstado, descripcion) {
    let accion, mensaje, color;
    
    switch(nuevoEstado) {
        case 1:
            accion = 'activar';
            mensaje = 'La ubicación estará disponible para asignar a cabañas';
            color = '#28a745';
            break;
        case 0:
            accion = 'desactivar';
            mensaje = 'La ubicación no estará disponible para nuevas asignaciones';
            color = '#dc3545';
            break;
        default:
    ...
    confirmar.then(confirmed => {
        if (confirmed) {
            ...
            .catch(error => {
                ...
                const errorMsg = 'Error al cambiar el estado de la ubicación: ' + error.message;
                if (typeof Swal !== 'undefined') {
                    Swal.fire('Error', errorMsg, 'error');
                } else {
                    alert(errorMsg);
...
function exportarUbicaciones(event) {
    if (event) {
    ...
    const baseExportUrl = '<?= url('/ubicaciones/exportar') ?>';
...
function exportarUbicacionesPDF(event) {
    if (event) {
        event.preventDefault();
    }
    ...
    const urlParams = new URLSearchParams(window.location.search);
    const basePdfUrl = '<?= url('/ubicaciones/exportar-pdf') ?>';
