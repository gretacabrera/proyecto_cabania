    ...
    <div class="card border-0 shadow-sm">
        ...
        <div class="card-header text-dark py-3 mb-0">
            <div class="row align-items-center">
                <div class="col">
                    <h4 class="mb-0">Gestión de Estados de Personas</h4>
                </div>
                <div class="col-auto">
                    <a href="<?= url('/estadospersonas/create') ?>" class="btn btn-primary btn-sm">
        ...
        <div class="card-body pb-0">
            <form method="GET" action="<?= url('/estadospersonas') ?>" class="mb-3">
                <div class="row g-2 align-items-end">
                    ...
                    <div class="col-auto">
                        ...
                        <input type="text" name="estadopersona_descripcion" class="form-control form-control-sm" 
                               placeholder="Buscar..." value="<?= htmlspecialchars($_GET['estadopersona_descripcion'] ?? '') ?>" style="width: 200px;">
                    </div>
                    <div class="col-auto ms-auto">
                        <label class="form-label small mb-1">Estado</label>
                        <select name="estadopersona_estado" class="form-select form-select-sm" style="width: 120px;">
                            <option value="">Todos</option>
                            <option value="1" <?= ($_GET['estadopersona_estado'] ?? '') == '1' ? 'selected' : '' ?>>Activo</option>
                            <option value="0" <?= ($_GET['estadopersona_estado'] ?? '') == '0' ? 'selected' : '' ?>>Inactivo</option>
                        </select>
                    </div>
                    <div class="col-auto">
                        <div class="btn-group">
                            ...
                            <a href="<?= url('/estadospersonas') ?>" class="btn btn-info btn-sm" title="Limpiar filtros">
        ...
        <div class="card-body p-0">
            <?php if (empty($estados)): ?>
                <div class="empty-state py-5 text-center">
                    ...
                    <h6 class="text-muted">No se encontraron estados de personas</h6>
                    <p class="text-muted small mb-3">Intenta modificar los filtros o crea un nuevo estado.</p>
                    <a href="<?= url('/estadospersonas/create') ?>" class="btn btn-outline-dark btn-sm">
            ...
            <?php else: ?>
                ...
                <div class="table-responsive">
                    <table id="tablaEstados" class="table table-hover mb-0">
                            ...
                            <?php foreach ($estados as $estado): ?>
                                <tr>
                                    <td class="border-0 py-3">
                                        <div class="fw-medium text-dark"><?= htmlspecialchars($estado['estadopersona_descripcion']) ?></div>
                                    </td>
                                    <td class="border-0 py-3">
                                        <?php if ($estado['estadopersona_estado'] == 1): ?>
                                    ...
                                    <td class="border-0 py-3 text-center">
                                        <div class="btn-group btn-group-sm" role="group">
                                            <a href="<?= url('/estadospersonas/' . $estado['id_estadopersona']) ?>"
                                               class="btn btn-outline-primary btn-sm"
                                               title="Ver detalle">
                                                <i class="fas fa-eye"></i>
                                            </a>
                                            <a href="<?= url('/estadospersonas/' . $estado['id_estadopersona'] . '/edit') ?>"
                                            ...
                                            <?php if ($estado['estadopersona_estado'] == 1): ?>
                                                <button class="btn btn-outline-danger btn-sm"
                                                        onclick="cambiarEstadoEstado(<?= $estado['id_estadopersona'] ?>, 0, '<?= addslashes($estado['estadopersona_descripcion']) ?>')"
                                                        title="Desactivar">
                                                    <i class="fas fa-ban"></i>
                                                </button>
                                            <?php else: ?>
                                                <button class="btn btn-outline-success btn-sm"
                                                        onclick="cambiarEstadoEstado(<?= $estado['id_estadopersona'] ?>, 1, '<?= addslashes($estado['estadopersona_descripcion']) ?>')"
                                                        title="Activar">
...
function cambiarEstadoEstado(id, nuevoEstado, descripcion) {
    ...
    confirmar.then(confirmed => {
        if (confirmed) {
            const url = `<?= url('/estadospersonas') ?>/${id}/estado`;
...
function exportarEstados(event) {
    ...
    const baseExportUrl = '<?= url('/estadospersonas/exportar') ?>';
...
function exportarEstadosPDF(event) {
    ...
    const urlParams = new URLSearchParams(window.location.search);
    const basePdfUrl = '<?= url('/estadospersonas/exportar-pdf') ?>';
