...
/**
 * Vista: Detalle de Tipo de Contacto
 * Descripción: Muestra información completa de un tipo de contacto
 * Autor: Sistema MVC
 * Fecha: 2025-11-10
 */

// Validar que existe el tipo de contacto
if (!isset($tipo_contacto) || empty($tipo_contacto)) {
    echo '<div class="alert alert-danger">Tipo de contacto no encontrado.</div>';
    return;
}
?>
...
<div class="content-wrapper">
    ...
    <div class="page-actions">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <div>
                <a href="<?= $this->url('/tiposcontactos') ?>" class="btn btn-primary">
                    <i class="fas fa-arrow-left"></i> Volver al listado
                </a>
            </div>
            <div class="action-buttons">
                <a href="<?= $this->url('/tiposcontactos/' . $tipo_contacto['id_tipocontacto']) . '/edit' ?>"
                    class="btn btn-warning">
                    <i class="fas fa-edit"></i> Editar Tipo de Contacto
                </a>
                
                <?php if ($tipo_contacto['tipocontacto_estado'] == 1): ?>
                    <!-- Tipo activo: puede desactivar -->
                    <button class="btn btn-danger ms-2"
                        onclick="cambiarEstadoTipoContacto(<?= $tipo_contacto['id_tipocontacto'] ?>, 0, '<?= addslashes($tipo_contacto['tipocontacto_descripcion']) ?>')">
                        <i class="fas fa-ban"></i> Desactivar
                    </button>
                <?php else: ?>
                    <!-- Tipo inactivo: solo puede activar -->
                    <button class="btn btn-success ms-2"
                        onclick="cambiarEstadoTipoContacto(<?= $tipo_contacto['id_tipocontacto'] ?>, 1, '<?= addslashes($tipo_contacto['tipocontacto_descripcion']) ?>')">
                    ...
                    <div class="row">
                        <div class="col-md-8">
                            <div class="info-group">
                                ...
                                <strong><?= htmlspecialchars($tipo_contacto['tipocontacto_descripcion']) ?></strong>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="info-group">
            ...
            <div class="card mt-3">
                ...
                <div class="card-body">
                    <div class="d-grid gap-2">
                        <a href="<?= $this->url('/tiposcontactos/create') ?>"
                            class="btn btn-outline-success btn-sm">
                            <i class="fas fa-plus"></i> Crear Nuevo Tipo
                        </a>
                        <a href="<?= $this->url('/tiposcontactos') ?>"
...
function cambiarEstadoTipoContacto(id, nuevoEstado, descripcion) {
    ...
    confirmar.then(confirmed => {
        if (confirmed) {
            const url = `<?= $this->url('/tiposcontactos') ?>/${id}/estado`;
