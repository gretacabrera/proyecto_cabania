    ...
    <div class="card border-0 shadow-sm">
        ...
        <div class="card-header text-dark py-3 mb-0">
            <div class="row align-items-center">
                ...
                <div class="col-auto">
                    <a href="<?= url('/huespedes/create') ?>" class="btn btn-primary btn-sm">
                        <i class="fas fa-plus me-1"></i>Nuevo Huésped
                    </a>
                </div>
            </div>
        </div>
        ...
        <div class="card-body pb-0">
            <form method="GET" action="<?= url('/huespedes') ?>" class="mb-3">
                <div class="row g-2 align-items-end">
                    ...
                    <div class="col-auto">
                        ...
                        <input type="text" name="persona_nombre" class="form-control form-control-sm" 
                               placeholder="" value="<?= htmlspecialchars($_GET['persona_nombre'] ?? '') ?>" style="width: 150px;">
                    </div>
                    <div class="col-auto">
                        <label class="form-label small mb-1">Apellido</label>
                        <input type="text" name="persona_apellido" class="form-control form-control-sm" 
                               placeholder="" value="<?= htmlspecialchars($_GET['persona_apellido'] ?? '') ?>" style="width: 150px;">
                    </div>
                    <div class="col-auto">
                    ...
                    <div class="col-auto ms-auto">
                        ...
                        <select name="huesped_estado" class="form-select form-select-sm" style="width: 120px;">
                            <option value="">Todos</option>
                            <option value="1" <?= ($_GET['huesped_estado'] ?? '') == '1' ? 'selected' : '' ?>>Activo</option>
                            <option value="0" <?= ($_GET['huesped_estado'] ?? '') == '0' ? 'selected' : '' ?>>Inactivo</option>
                        </select>
                    </div>
                    <div class="col-auto">
                        <div class="btn-group">
                            ...
                            <a href="<?= url('/huespedes') ?>" class="btn btn-info btn-sm" title="Limpiar filtros">
                ...
                <div class="row mt-4">
                    ...
                    <div class="col-auto">
                        <div class="btn-group" role="group">
                            <button type="button" onclick="exportarHuespedes(event)" class="btn btn-success btn-sm" title="Exportar a Excel">
                                <i class="fas fa-file-excel me-1"></i> Excel
                            </button>
                            <button type="button" onclick="exportarHuespedespdf(event)" class="btn btn-danger btn-sm" title="Exportar a PDF">
        ...
        <div class="card-body p-0">
            <?php if (empty($huespedes)): ?>
                <div class="empty-state py-5 text-center">
                    ...
                    <h6 class="text-muted">No se encontraron huéspedes</h6>
                    <p class="text-muted small mb-3">Intenta modificar los filtros o crea un nuevo huésped.</p>
                    <a href="<?= url('/huespedes/create') ?>" class="btn btn-outline-dark btn-sm">
                        <i class="fas fa-plus fa-sm"></i> Crear huésped
                    </a>
                </div>
            <?php else: ?>
                ...
                <div class="table-responsive">
                    <table id="tablaHuespedes" class="table table-hover mb-0">
                        <thead class="thead-light">
                            <tr>
                                ...
                                <th class="border-0 py-3">Ubicación</th>
                                ...
                                <tr>
                                    ...
                                    <td class="border-0 py-3 text-center">
                                        <div class="btn-group btn-group-sm" role="group">
                                            <a href="<?= url('/huespedes/' . $huesped['id_huesped']) ?>"
                                               class="btn btn-outline-primary btn-sm"
                                               title="Ver detalle">
                                                <i class="fas fa-eye"></i>
                                            </a>
                                            <a href="<?= url('/huespedes/' . $huesped['id_huesped']) . '/edit'?>"
...
function cambiarEstadoHuesped(id, nuevoEstado, nombre) {
    ...
    confirmar.then(confirmed => {
        if (confirmed) {
            const url = `<?= url('/huespedes') ?>/${id}/estado`;
...
function exportarHuespedes(event) {
    if (event) {
    ...
    const baseExportUrl = '<?= url('/huespedes/exportar') ?>';
...
function exportarHuespedespdf(event) {
    if (event) {
        event.preventDefault();
    }
    ...
    const urlParams = new URLSearchParams(window.location.search);
    const basePdfUrl = '<?= url('/huespedes/exportar-pdf') ?>';
