    ...
    <div class="card border-0 shadow-sm">
        ...
        <div class="card-header text-dark py-3 mb-0">
            <div class="row align-items-center">
                ...
                <div class="col-auto">
                    <a href="<?= url('/perfiles/create') ?>" class="btn btn-primary btn-sm">
        ...
        <div class="card-body pb-0">
            <form method="GET" action="<?= url('/perfiles') ?>" class="mb-3">
                <div class="row g-2 align-items-end">
                    ...
                    <div class="col-auto">
                        ...
                        <input type="text" name="perfil_descripcion" class="form-control form-control-sm" 
                               placeholder="" value="<?= htmlspecialchars($_GET['perfil_descripcion'] ?? '') ?>" style="width: 200px;">
                    </div>
                    <div class="col-auto ms-auto">
                        <label class="form-label small mb-1">Estado</label>
                        <select name="perfil_estado" class="form-select form-select-sm" style="width: 120px;">
                            <option value="">Todos</option>
                            <option value="1" <?= ($_GET['perfil_estado'] ?? '') == '1' ? 'selected' : '' ?>>Activo</option>
                            <option value="0" <?= ($_GET['perfil_estado'] ?? '') == '0' ? 'selected' : '' ?>>Inactivo</option>
                        </select>
                    </div>
                    <div class="col-auto">
                        <div class="btn-group">
                            ...
                            <a href="<?= url('/perfiles') ?>" class="btn btn-info btn-sm" title="Limpiar filtros">
                ...
                <div class="row mt-4">
                    ...
                    <div class="col-auto">
                        <div class="btn-group" role="group">
                            <button type="button" onclick="exportarPerfiles(event)" class="btn btn-success btn-sm" title="Exportar a Excel">
                                <i class="fas fa-file-excel me-1"></i> Excel
                            </button>
                            <button type="button" onclick="exportarPerfilesPDF(event)" class="btn btn-danger btn-sm" title="Exportar a PDF">
        ...
        <div class="card-body p-0">
            <?php if (empty($perfiles)): ?>
                <div class="empty-state py-5 text-center">
                    <div class="mb-4">
                        <i class="fas fa-users-cog fa-3x text-muted opacity-50"></i>
                    </div>
                    <h6 class="text-muted">No se encontraron perfiles</h6>
                    <p class="text-muted small mb-3">Intenta modificar los filtros o crea un nuevo perfil.</p>
                    <a href="<?= url('/perfiles/create') ?>" class="btn btn-outline-dark btn-sm">
                        <i class="fas fa-plus fa-sm"></i> Crear perfil
                    </a>
                </div>
            <?php else: ?>
                ...
                <div class="table-responsive">
                    <table id="tablaPerfiles" class="table table-hover mb-0">
                            ...
                            <?php foreach ($perfiles as $perfil): ?>
                                <tr>
                                    <td class="border-0 py-3">
                                        <div class="fw-medium text-dark"><?= htmlspecialchars($perfil['perfil_descripcion']) ?></div>
                                    </td>
                                    <td class="border-0 py-3">
                                    ...
                                    <td class="border-0 py-3 text-center">
                                        <div class="btn-group btn-group-sm" role="group">
                                            <a href="<?= url('/perfiles/' . $perfil['id_perfil']) ?>"
...
function cambiarEstadoPerfil(id, nuevoEstado, nombre) {
    ...
    const texto = nuevoEstado === 1 
        ? `El perfil "${nombre}" volverá a estar disponible.`
        : `El perfil "${nombre}" ya no estará disponible.`;
...
// Función para exportar perfiles a Excel (.xlsx)
function exportarPerfiles(event) {
    if (event) {
    ...
    const baseExportUrl = '<?= url('/perfiles/exportar') ?>';
...
// Función para exportar perfiles a PDF
function exportarPerfilesPDF(event) {
    if (event) {
        event.preventDefault();
    }
    ...
    const urlParams = new URLSearchParams(window.location.search);
    const basePdfUrl = '<?= url('/perfiles/exportar-pdf') ?>';
