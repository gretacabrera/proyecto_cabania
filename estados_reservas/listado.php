...
<?php
	...
	while ($row = $registros->fetch_assoc()) {
		...
		if ($row["estadoreserva_estado"]) {
			...
			echo "<button class='abm-button baja-button' onclick='confirmarEliminacion(\"baja_logica.php?id_estadoreserva=".$row["id_estadoreserva"]."\", \"dar de baja esta categoría\")'>Eliminar</button>";
		} else {
			// Si está de baja, mostrar botón Recuperar
			echo "<button class='abm-button alta-button' onclick='confirmarEliminacion(\"quitar_baja_logica.php?id_estadoreserva=".$row["id_estadoreserva"]."\", \"recuperar esta categoría\")'>Recuperar</button>";
		}
